...
class svFSI(Simulation):
    ...
    def set_solid(self, n, t):
        ...
        # Compute WSS gradient along vessel (z-direction)
        wss_grad = self.compute_axial_wss_gradient(wss_mag)

        # Store WSS gradient in props[:, 6] (replacing magnitude)
        props[:, 6] = wss_grad

        # Initialize homeostatic WSS gradient (tauo) at first timestep
        # This ensures tauo matches the gradient values, not old magnitude values
        if t == 0 and n == 0:
            props[:, 3] = wss_grad  # Set tauo = tau at prestress
