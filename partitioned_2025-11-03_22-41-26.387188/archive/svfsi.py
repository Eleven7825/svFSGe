...
class svFSI(Simulation):
    ...
    def compute_axial_wss_gradient(self, wss_magnitude):
        """
        Compute normalized axial WSS gradient for growth stimulus.

        Calculates (∂||WSS||/∂z) / ||WSS|| - a dimensionless measure of the relative
        spatial variation of wall shear stress along the vessel axis.
        ...
        Returns:
            normalized_gradient: (N,) array of (∂WSS/∂z) / WSS (dimensionless, units: 1/mm)
        """
        # Get mesh parameters
        ...
        # Normalize by WSS magnitude to get dimensionless relative variation
        # Use max(WSS, WSS_min) to prevent division by near-zero values
        # WSS_min is set to 1% of mean WSS to handle near-zero regions robustly
        wss_mean = np.mean(wss_magnitude[wss_magnitude > 1e-10])
        wss_min = max(0.01 * wss_mean, 1e-6)  # At least 1e-6 to prevent true zeros

        normalized_gradient = wss_gradient / np.maximum(wss_magnitude, wss_min)
    ...
    def set_solid(self, n, t):
        ...
        # Initialize gr_properties on first call (fix corrupted initial values)
        if t == 0 and n == 0:
            # Reset all values to ensure clean start
            props[:, 0] = 1.0   # Jo: initial Jacobian = 1 (no deformation)
            props[:, 1] = 0.0   # svo: initial stress (will be set during prestress)
            props[:, 2] = 0.0   # Reserved
            props[:, 3] = 0.0   # tauo: will be set from tau during prestress
            props[:, 4:6] = 0.0  # Reserved
            props[:, 6] = 0.0   # tau: current stimulus (set below)
            props[:, 7] = 0.0   # time
        ...
        # Compute normalized WSS gradient: (∂WSS/∂z) / WSS
        # This is dimensionless and represents relative spatial variation
        norm_wss_grad = self.compute_axial_wss_gradient(wss_mag)

        # Store normalized WSS gradient in props[:, 6] (tau)
        props[:, 6] = norm_wss_grad

        # During prestressing (t=0), also set tauo equal to tau initially
        # The C++ code will update tauo when load_step_flag is True
        if t == 0:
            props[:, 3] = norm_wss_grad
