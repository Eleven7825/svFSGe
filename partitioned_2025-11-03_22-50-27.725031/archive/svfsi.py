...
class svFSI(Simulation):
    ...
    def set_solid(self, n, t):
        ...
        # Initialize WSS-related properties on first call
        # IMPORTANT: Preserve Jo (col 0) and svo (col 1) from mesh - these contain
        # prestress state from previous run. Only reset tau/tauo since we're changing
        # the growth stimulus from WSS magnitude to WSS gradient.
        if t == 0 and n == 0:
            # Keep Jo and svo from original mesh (cols 0-1)
            # Reset WSS stimulus values only:
            props[:, 3] = 0.0   # tauo: will be set from tau below
