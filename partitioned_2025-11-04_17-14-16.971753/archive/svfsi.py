...
class svFSI(Simulation):
    ...
    def set_solid(self, n, t):
        ...
        # Initialize gr_properties on first call
        # The mesh generation incorrectly uses gr_properties to store geometric
        # coordinates, not actual growth properties. We must reset everything.
        if t == 0 and n == 0:
            props[:, 0] = 1.0    # Jo: initial Jacobian (undeformed state)
            props[:, 1] = 0.0    # svo: initial stress (will be set during prestress)
            props[:, 2] = 0.0    # Reserved
            props[:, 3] = 0.0    # tauo: homeostatic stimulus (set from tau below)
            props[:, 4:6] = 0.0  # Reserved
            props[:, 6] = 0.0    # tau: current stimulus (set below from WSS gradient)
            props[:, 7] = 0.0    # time
            props[:, 8] = 0.0    # Reserved
            props[:, 9:12] = 0.0 # WSS gradient vector components
            props[:, 12] = 0.0   # load step flag (set below)

        # Get WSS magnitude
        wss_mag = self.curr.get(("solid", "wss", "vol"))

        # Compute WSS gradient magnitude: ∂WSS/∂z (Pa/mm)
        wss_grad = self.compute_axial_wss_gradient(wss_mag)

        # Apply scaling factor to match expected value range
        # The growth model was calibrated with WSS magnitude (~0.6 Pa)
        # WSS gradient is ~0.001 Pa/mm, so we scale by ~500 to match
        if "growth" in self.p and "wss_gradient_scale" in self.p["growth"]:
            scale_factor = self.p["growth"]["wss_gradient_scale"]
            wss_grad_scaled = wss_grad * scale_factor
            if t == 0 and n == 0:
                print(f"Applying WSS gradient scaling factor: {scale_factor}")
                print(f"  Raw gradient range: [{wss_grad.min():.6f}, {wss_grad.max():.6f}] Pa/mm")
                print(f"  Scaled gradient range: [{wss_grad_scaled.min():.6f}, {wss_grad_scaled.max():.6f}]")
        else:
            # Default: no scaling (use raw gradient)
            wss_grad_scaled = wss_grad
            if t == 0 and n == 0:
                print("No WSS gradient scaling applied (using raw gradient)")

        # Store scaled WSS gradient in props[:, 6] (tau - current growth stimulus)
        props[:, 6] = wss_grad_scaled

        # At first iteration, set tauo = tau to avoid division by zero
        # C++ code will update tauo at end of prestressing, but it needs a non-zero
        # value initially to compute tau_ratio = tau / tauo
        if t == 0 and n == 0:
        ...
        # When True, C++ growth code stores current tau as homeostatic tauo
        props[:, 12] = n == 0

        add_array(solid, props, name)

        # Add WSS gradient arrays for visualization in ParaView
        add_array(solid, wss_grad, "WSS_Gradient")  # Raw gradient in Pa/mm
        add_array(solid, wss_grad_scaled, "WSS_Gradient_Scaled")  # Scaled gradient used as stimulus
        add_array(solid, wss_mag, "WSS_Magnitude")  # WSS magnitude for comparison
